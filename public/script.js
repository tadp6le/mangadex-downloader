...
(() => {
  ...
  /* ---------- State ---------- */
  const state = {
    mangaTitle: '',   // remembered so downloads can be named "Title - Ch. X.cbz"
    chapters: [],
    filtered:  [],
    selected:  new Set(),
  ...
  */
  function downloadChapter(chapterId, label) {
    ...
    // Look up the chapter number so the server can name the file "Title - Ch. X.cbz"
    const ch = state.chapters.find((c) => c.id === chapterId);
    const params = new URLSearchParams({
      downloadId,
      title:   state.mangaTitle || '',
      chapter: (ch && ch.chapter) ? ch.chapter : '',
    });
    ...
    a.href = `/api/download/${chapterId}?${params.toString()}`;
    a.rel = 'noopener';
